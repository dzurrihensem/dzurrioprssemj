...
const GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycby-0ymw18O9Pjt6j9FcRemwNUvNu3oJIekWC8raOP7a6WUMO_qLYvyjTqdDLRy-HX_lIQ/exec"; 
...
const App: React.FC = () => {
  ...
  const handleReset = () => {
    if (window.confirm("Adakah anda pasti untuk memadam draf ini? Semua data akan hilang.")) {
  ...
  const handleSubmit = async () => {
    if (!reportData.tajuk || !reportData.reporterName || reportData.images.length === 0 || !reportData.logo) {
      alert("Sila lengkapkan maklumat wajib (Tajuk, Nama Pelapor, Logo & Gambar).");
      return;
    }
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        <header className="text-center mb-12 text-white">
          <div className="inline-block p-6 bg-white rounded-[3rem] mb-6 shadow-[0_25px_50px_rgba(0,0,0,0.2)] border-4 border-white/40 ring-1 ring-black/5 hover:scale-105 transition-transform duration-500">
            {reportData.logo ? (
              ...
              <div className="w-32 h-32 flex items-center justify-center bg-gray-100 rounded-2xl text-gray-300 text-[11px] font-black uppercase text-center p-4">Muat Naik Logo</div>
            )}
          </div>
        ...
        <nav className="flex flex-col sm:flex-row justify-center items-center gap-5 mb-10">
          ...
          {view === 'form' && (
            <button
              onClick={handleReset}
              className="flex items-center gap-3 px-8 py-4 rounded-[1.5rem] font-black uppercase text-[10px] bg-red-500/20 text-red-200 hover:bg-red-500 hover:text-white border border-red-500/30 backdrop-blur-md shadow-lg"
            >
        ...
        {showSuccess && (
          <div className="mb-10 bg-white/95 backdrop-blur-2xl p-10 rounded-[3.5rem] border-4 border-green-500 shadow-2xl flex flex-col sm:flex-row items-center gap-8 animate-in zoom-in">
              ...
              <button 
                ...
                className="px-10 py-5 bg-indigo-600 text-white font-black text-xs uppercase rounded-2xl shadow-xl"
              >
                Ke Arkib
              </button>
          </div>
